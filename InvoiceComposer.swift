...
class InvoiceComposer: NSObject {
    ...
    let pathToInvoiceHTMLTemplate = Bundle.main.path(forResource: "invoice2", ofType: "html")
    ...
    func exportHTMLContentToPDF(HTMLContent: String) {
        ...
        let dataFolderURL = "\(bundleURL.appendingPathComponent("docs"))"
        
        pdfFilename = "\(AppDelegate.getAppDelegate().getDocDir())/Invoice\(invoiceNumber!).pdf"
       // pdfFilename2 = "\(bundleURL)invoice12.pdf"
        pdfData?.write(toFile: pdfFilename, atomically: true)
        
       // pdfData?.write(toFile: pdfFilename2, atomically: true)
